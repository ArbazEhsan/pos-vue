  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        ...
        <p class="category"> Here you create your Purchase Invoice</p>
      </div>
      <div class="card-body">
          ...
          <table class="table">
            <tbody>
              <tr>
                ...
                <td>Vendor Name: <div class="input-group"><!-- <input type="text" name="cust_name" id="cust_name" list="cust_list" placeholder="-- Select --" class="form-control" autocomplete="off" required> -->
                <select name="cust_name" id="cust_name" class="form-control" required><!-- <option value="0">-- Select --</option> -->
                  <?php echo $account; ?>
                </select></div><!-- <datalist id="cust_list"><?php echo $account; ?></datalist> --></td>                
              </tr>
            </tbody> 
          </table>
        </div>
          ...
          <table class="table">
            <tbody>
              <tr>
                ...
                <td>Inv. Disc: <div class="input-group"><input type="text" name="discount" id="discount" value="0" placeholder="Inv Discount" class="form-control"></div></td>
...
<script type="text/javascript">
  ...
  function searchFunction(txt,txt2,rowNum,from) {
    $.ajax({
      type: "GET",
      url:"/searchProduct/"+txt+"/"+txt2+"/"+rowNum+"/purchase",
  ...
  $(document).ready(function (argument) {
    $('#lockForm').on('submit',function (e) {
      ...
      $.ajax({
        type: "POST",
        url:"/purchaseInvoice/"+e+"/"+f+"/"+g+"/"+h+"/sale",
        data: $('#lockForm').serialize(),
        success:function(info){
        //   loadDoc('purPrint',info);
            window.open('/purPrint/'+info,'popUpWindow','height=400,width=1000,left=50,top=50,resizable=yes,scrollbars=yes,toolbar=yes,menubar=no,location=no,directories=no, status=yes');
            location.reload();
        },
        error: function (error) {
          //console.log(error);
